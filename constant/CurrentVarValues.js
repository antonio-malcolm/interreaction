...
let SHOULD_INCLUDE_INTERREACTION = process.env[VarNames.SHOULD_INCLUDE_INTERREACTION];

if (SHOULD_INCLUDE_INTERREACTION === 'true') {
  SHOULD_INCLUDE_INTERREACTION = true;
} else {
  if (SHOULD_INCLUDE_INTERREACTION !== true) {
    SHOULD_INCLUDE_INTERREACTION = false;
  }
}

let WITH_CUSTOM_TAGNAME = String(
    process.env[VarNames.WITH_CUSTOM_TAGNAME]
  );

if (WITH_CUSTOM_TAGNAME === 'true') {
  WITH_CUSTOM_TAGNAME = true;
} else {
  if (WITH_CUSTOM_TAGNAME !== true) {
    WITH_CUSTOM_TAGNAME = false;
  }
}
