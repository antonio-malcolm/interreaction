...
const sanitizeShouldIncludeInterReactionArg = function(shouldIncludeInterReactionArg, taskName) {
  validateArgDefinedNonEmpty(
    taskName,
    'task name',
    'shouldIncludeInterReactionArg'
  );

  if (typeof shouldIncludeInterReactionArg === 'boolean') {
    return shouldIncludeInterReactionArg;
  }

  if (shouldIncludeInterReactionArg === 'true') {
    return true;
  }

  console.log(
    `No shouldIncludeInterReaction arg provided, for task "${taskName}" - defaulting, to false.`
  );

  return false;
};

const sanitizeWithCustomTagNameArg = function(withCustomTagNameArg, taskName) {
  validateArgDefinedNonEmpty(
    taskName,
    'task name',
    'withCustomTagNameArg'
  );

  if (typeof withCustomTagNameArg === 'boolean') {
    return withCustomTagNameArg;
  }

  if (withCustomTagNameArg === 'true') {
    return true;
  }

  console.log(
    `No withCustomTagNameArg arg provided, for task "${taskName}" - defaulting, to false.`
  );
...
module.exports = Object.freeze({
  ...
  sanitizeShouldHotReloadArg,
  sanitizeShouldNetworkArg,
  sanitizeShouldIncludeInterReactionArg,
  sanitizeWithCustomTagNameArg,
  sanitizeWorkspaceArg,
  validateArgDefinedNonEmpty
});
