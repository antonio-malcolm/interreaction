...
const getArgs = function(argKeys) {
  ...
  let parsedArgs = parseArgs(
      process.argv.slice(3),
      {
        ...
        'boolean': [
          ...
          VarNames.SHOULD_INCLUDE_INTERREACTION,
          VarNames.WITH_CUSTOM_TAGNAME
        ]
      }
    );
  ...
  if (!CommonUtils.isNonEmptyObject(parsedArgs)) {
    ...
    parsedArgs[VarNames.SHOULD_INCLUDE_INTERREACTION] = process.env[VarNames.SHOULD_INCLUDE_INTERREACTION];
    parsedArgs[VarNames.WITH_CUSTOM_TAGNAME] = process.env[VarNames.WITH_CUSTOM_TAGNAME];
  }
  ...
  argKeys.forEach((key) => {
    switch (key) {
      ...
      case VarNames.SHOULD_INCLUDE_INTERREACTION:
        if (parsedArgs[key] !== true) {
          args.push(false);
        } else {
          args.push(true);
      ...
      case VarNames.WITH_CUSTOM_TAGNAME:
        if (parsedArgs[key] !== true) {
          args.push(false);
        } else {
          args.push(true);
...
const start = function(
  ...
  try {
    ...
    shouldIncludeInterReaction = TaskArgSanitizer.sanitizeShouldIncludeInterReactionArg(
        shouldIncludeInterReaction,
        Tasks.START
      );
    withCustomTagName = TaskArgSanitizer.sanitizeWithCustomTagNameArg(
        withCustomTagName,
        Tasks.START
      );
  } catch (ex) {
